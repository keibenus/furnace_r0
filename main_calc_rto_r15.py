...
CONFIG = {
    ...
    'solid': {
        'mono': {
            'rho': 2500.0, 'c': 1050.0, 'k': 1.5,
            'eps': m_eps, 'd': m_d, 'av': m_av, 'phi': m_phi
        },
        'pack': {
            'rho': 2350.0, 'c': 950.0, 'k': 1.3,
            'eps': p_eps, 'd': p_d, 'av': p_av, 'phi': p_phi
        }
    }
}
...
class RTOCanister:
    def __init__(self):
        ...
        # 物性値マッピング
        mono = self.p['solid']['mono']
        pack = self.p['solid']['pack']
        
        self.eps = np.concatenate([np.full(N_m, mono['eps']), np.full(N_p, pack['eps'])])
        self.d   = np.concatenate([np.full(N_m, mono['d']),   np.full(N_p, pack['d'])])
        self.av  = np.concatenate([np.full(N_m, mono['av']),  np.full(N_p, pack['av'])])
        
        # 配列化された固体物性
        self.rho_s = np.concatenate([np.full(N_m, mono['rho']), np.full(N_p, pack['rho'])])
        self.c_s   = np.concatenate([np.full(N_m, mono['c']),   np.full(N_p, pack['c'])])
        self.k_s   = np.concatenate([np.full(N_m, mono['k']),   np.full(N_p, pack['k'])])
...
class RTOSystem:
    ...
    def calc_bed(self, y, bed, u, rho, cp, k, mu, T_in, dir):
        ...
        # s['k'] ではなく bed.k_s (配列) を使う
        term_cond = (1.0 - bed.eps) * bed.k_s * d2Ts
        term_ht   = h * bed.av * (Tg - Ts)
        dTs_dt = (term_cond + term_ht) / ((1.0 - bed.eps) * bed.rho_s * bed.c_s)
    ...
    def run(self):
        print(f"--- Simulation Start (Material Properties Fixed) ---")
        ...
        for i in range(self.p['n_cycles']):
            ...
            while t_cycle < t_end:
                ...
                if self.flow_dir == 1: 
                    T_cc_in = g1[-1]; cp_in = self.phys.get_air_properties(np.array([T_cc_in]))[1][0]
                else:
                    T_cc_in = g2[-1]; cp_in = self.phys.get_air_properties(np.array([T_cc_in]))[1][0]
...
if __name__ == "__main__":
    ...
    # --- Plotting ---
    N = sim.bed1.N
    x1 = sim.bed1.x; x2 = sim.bed2.x
    p0 = sim.profile_data_t0; p90 = sim.profile_data_t90
    ...
    def setup_ax(axL, axR, title, ylabel):
        ...
        axR.set_xlim(1.35, 0); axR.set_title("Canister 2 (Right)"); axR.set_yticklabels([])

    # 1. Gas Temp Compare
    fig1, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5), gridspec_kw={'wspace': 0.05})
    setup_ax(ax1, ax2, "", "Gas Temp [K]")
    ax1.plot(x1, p0['g1'], 'b-', label='Gas t=0')
    ax1.plot(x1, p90['g1'], 'r--', label='Gas t=90')
    ax1.legend(loc='upper left')
    ax1.set_ylim(300, 1200)
    ax2.plot(x2, p0['g2'], 'b-')
    ax2.plot(x2, p90['g2'], 'r--')
    ax2.set_ylim(300, 1200)
    plt.savefig('rto_gas_compare.png')

    # 2. Solid Temp Compare
    fig2, (ax3, ax4) = plt.subplots(1, 2, figsize=(10, 5), gridspec_kw={'wspace': 0.05})
    setup_ax(ax3, ax4, "", "Solid Temp [K]")
    ax3.plot(x1, p0['s1'], 'b-', label='Solid t=0')
    ax3.plot(x1, p90['s1'], 'r--', label='Solid t=90')
    ax3.legend(loc='upper left')
    ax3.set_ylim(300, 1200)
    ax4.plot(x2, p0['s2'], 'b-')
    ax4.plot(x2, p90['s2'], 'r--')
    ax4.set_ylim(300, 1200)
    plt.savefig('rto_solid_compare.png')
    
    # 3. Heat Transfer Coeff
    fig3, (ax5, ax6) = plt.subplots(1, 2, figsize=(10, 5), gridspec_kw={'wspace': 0.05})
    setup_ax(ax5, ax6, "", "h [W/m2K]")
    ax5.plot(x1, p90['h1'], 'r-', label='t=90')
    ax6.plot(x2, p90['h2'], 'r-')

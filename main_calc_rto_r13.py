...
m_eps = 0.70    # 空隙率
m_d   = 0.005   # 流路代表径 [m]
m_phi = 1.0     # 球形度 (モノリスは1.0)
# モノリスの比表面積 (正方形セル幾何計算: 4 * eps / d)
m_av  = (4.0 * m_eps) / m_d  

# [Packing (Saddles)]
p_eps = 0.70    # 空隙率 (サドルは空隙が大きいので 0.70-0.75 程度が実態に近い)
p_d   = 0.025   # 代表径 [m]
p_phi = 0.3     # 球形度 (論文指定値)
# 充填層の比表面積 (論文式変形: av = 6*(1-eps) / (phi*d))
...
CONFIG = {
    ...
    'solid': {
        ...
        # 変数を参照
        'mono': {'eps': m_eps, 'd': m_d, 'av': m_av, 'phi': m_phi}, 
        'pack': {'eps': p_eps, 'd': p_d, 'av': p_av, 'phi': p_phi}
    }
}

# 確認用プリント (実行時に計算された値が表示されます)
print(f"--- Calculated Parameters ---")
print(f"Monolith: d={m_d*1000:.1f}mm, eps={m_eps}, av={m_av:.1f} m2/m3")
print(f"Packing : d={p_d*1000:.1f}mm, eps={p_eps}, av={p_av:.1f} m2/m3 (phi={p_phi})")
print(f"-----------------------------")
